...
Mayer-Vietoris}, que presentaremos en el capítulo \refchapter{MayerVietoris}.
...
\draw[fill=blue, fill opacity=0.5] (-2,-1) circle (2cm);
\draw (-4,1) node {$\Omega_1$};

\draw[thick] (-2,-1) arc (15:114:1);

\draw[fill=green, fill opacity=0.5] (2,1) circle (2cm);
\draw (0,3) node {$\Omega_2$};

\draw[thick] (2,1) arc (0:95:1);

\draw[thick, dashed] (-2,-1) -- (2,1);
\end{tikzpicture}
\caption{\labfig{FigPentaHexa}Los
...
\begin{proposition}
\labprop{SumaDirArco} Sea $X$ un espacio topológico y $\{X_\alpha\colon \alpha \in
...
\begin{remark}\labremark{H0CuentaComponentes}
...
\refremark{H0CuentaComponentes}. Por tanto, nos queda probar la segunda igualdad.
...
Sea $z \in Z_p(X)$. Usando \refeq{IdTp}, $z=\p T_p(z)+T_p(\p z)=\p T_p(z)$,
...
\draw (5,2.5) node {$(1,1)$};
\draw (0,2.5) node {$\left(\frac{1}{2},1\right)$};
\draw (10/3-5,2.5) node {$\left(\frac{1}{3},1\right)$};
\draw (10/6-5,2.5) node {$\dots$};
\draw (-5,2.5) node {$(0,1)$};
...
\foreach \x in {1,...,15}
{
\draw (10/\x-5,2) -- (10/\x-5,0);
}

\draw (1/3-5,1) node {$\dots$};

\draw (5,-0.5) node {$(1,0)$};
\draw (0,-0.5) node {$\left(\frac{1}{2},0\right)$};
\draw (10/3-5,-0.5) node {$\left(\frac{1}{3},0\right)$};
\draw (10/6-5,-0.5) node {$\dots$};
\draw (-5,-0.5) node {$(0,0)$};
\end{tikzpicture}
\caption{\labfig{Peine}Primeras $15$ iteraciones del peine del topólogo.}
\end{marginfigure}
...
\begin{remark}[Contráctil implica deformable]
...
\begin{corollary}\labcor{Contractil}
...
Para ver esta identidad, sea $c \in Z_p(C)$. Usando que \refeq{SecExactafg} es
